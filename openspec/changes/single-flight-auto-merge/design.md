...
> **实现细化（apply 2026-07-30，task 4.4）**：落地为 `circuit_breaker.py`——per-owner_repo cooldown journal（`state_dir/cooldown/<safe>.journal.jsonl`，跨 run 存活），`record_revert`（REVERTED 后追加 `revert_recorded` 事件）+ `is_in_cooldown`（merge 前查窗口内匹配）。键 = `_coord.circuit_key`（**task 4.4 fix 2026-07-30**：slug-based `prd_id(stable_slug, prd_digest)`——`stable_slug`（frontmatter 语义 slug）+ 内容 digest 两因子都不含 cron stamp → 跨 cron 稳定；内容改→新 digest→新键→放行重试，合理：改了可能已修。旧 path-based `_coord.prd_id`（`prd_id(prd_path, digest)`）含 stamp（`{stamp}_{slug}.md`）跨 cron 变，致熔断跨 cron 失效、本护栏防不住「夜夜复发」，canary 判据 c 暴露，已弃用做熔断键）。**fail-open**：熔断是「额外阻止」型护栏（非破坏性副作用准入门），无正向匹配证据→放行（journal 损坏/读失败→False），与 rebase/merge/revert 的 fail-safe（无证据→UNKNOWN→block）刻意相反。durable 告警化（损坏提醒）留 4.5。
