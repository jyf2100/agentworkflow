...
Make the project pipeline reproducible and verifiable in any clean environment. Pin Python versions, runtime and development dependencies, and test configuration in version-controlled metadata; expose a single quality command that runs syntax, unit, and static checks; gate the default branch in CI; and ensure control-plane tests run hermetically against minimal Node and Python target fixtures.
