...
def build_prompt(args: dict, prd_text: str, branch: str | None) -> str:
    ...
    base_prompt = "\n".join([
        ...
        "   **测试命令须纯净**：发布证据只采**裸测试命令**（无 `|` `>` `&` `;` 的管道/重定向/链式）——",
        "   只有这类命令的退出码可信，带管道的变体（如 `npm test | tail`）退出码失真会被拒收。",
        "   看长输出时**先纯净跑一次**（如 `npm test` / `pytest`）留退出码证据，再用带管道的命令查看尾部；",
        "   别只跑带管道的变体，否则证据采不到、会被判「未测试」而阻断发布。",
